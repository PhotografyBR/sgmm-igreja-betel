...
export default function UsersPage() {
  ...
  async function deleteUser(u) {
    ...
    if (!window.confirm(`Remover "${u.name}" do sistema? Esta ação não pode ser desfeita.`)) return;
    try {
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ color: '#6B7280', fontSize: 14, marginTop: 2 }}>
            {users.length} {users.length === 1 ? 'membro cadastrado' : 'membros cadastrados'}
          </p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          {currentUser?.role === 'admin' && (
            <button onClick={openNew} style={{
              ...
              color: 'white', border: 'none', borderRadius: 10, padding: '9px 20px',
          ...
          byRole[role].length > 0 && (
            <div key={role} style={{ marginBottom: 28 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                ...
                <h2 style={{ fontSize: 13, fontWeight: 700, color: meta.color, textTransform: 'uppercase', letterSpacing: '0.6px' }}>
                  {meta.label}
                </h2>
                <span style={{ background: meta.bg, color: meta.color, borderRadius: 10, padding: '1px 8px', fontSize: 12, fontWeight: 600 }}>
                  {byRole[role].length}
                </span>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 10 }}>
                {byRole[role].map(u => (
                  <div key={u.id} style={{
                    ...
                    border: u.id === currentUser?.id ? `2px solid ${meta.color}` : '2px solid transparent',
                    position: 'relative'
                  }}>
                    {u.id === currentUser?.id && (
                      <span style={{
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontSize: 14, fontWeight: 600, color: '#1F2937', marginBottom: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {u.name}
                      </p>
                      ...
                      {u.phone && (
                        <p style={{ fontSize: 11, color: '#9CA3AF', marginTop: 1 }}>{u.phone}</p>
                      )}
                    </div>
                    {currentUser?.role === 'admin' && (
                      <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
                        <button onClick={() => openEdit(u)} style={{
                          ...
                          padding: '5px 10px', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit', color: '#374151'
                        }}>
                          Editar
                        </button>
                        {u.id !== currentUser?.id && (
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: 20 }}>
          <div style={{ background: 'white', borderRadius: 16, padding: 28, width: '100%', maxWidth: 460 }} className="fade-in">
            <h2 style={{ fontSize: 18, fontWeight: 700, marginBottom: 6, color: '#1E1B4B' }}>
              {editing ? 'Editar membro' : 'Novo membro'}
            </h2>
            <p style={{ fontSize: 13, color: '#6B7280', marginBottom: 20 }}>
              {editing ? 'Atualize os dados do membro abaixo.' : 'Preencha os dados para criar o acesso.'}
            </p>
            <form onSubmit={handleSubmit}>
              <div style={{ display: 'grid', gap: 14 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 5 }}>Nome completo *</label>
                  <input type="text" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} required style={inputStyle} placeholder="Ex: João da Silva" />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 5 }}>Email *</label>
                  <input type="email" value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} required style={inputStyle} placeholder="email@exemplo.com" />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 5 }}>
                    {editing ? 'Nova senha (deixe vazio para manter)' : 'Senha *'}
                  </label>
                  <input type="password" value={form.password} onChange={e => setForm(p => ({ ...p, password: e.target.value }))} required={!editing} style={inputStyle} placeholder="Mínimo 6 caracteres" />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 5 }}>Telefone</label>
                  <input type="tel" value={form.phone} onChange={e => setForm(p => ({ ...p, phone: e.target.value }))} style={inputStyle} placeholder="(00) 00000-0000" />
                </div>
                <div>
                  ...
                  <div style={{ marginTop: 8, padding: '8px 12px', background: ROLES[form.role]?.bg, borderRadius: 8 }}>
                    ...
                    {ROLES[form.role]?.permissions.map((p, i) => (
                      <p key={i} style={{ fontSize: 11, color: ROLES[form.role]?.color, opacity: 0.85 }}>• {p}</p>
                    ))}
                  </div>
                </div>
              </div>
      ...
      {showPermissions && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: 20 }}>
          <div style={{ background: 'white', borderRadius: 16, padding: 28, width: '100%', maxWidth: 520 }} className="fade-in">
                ...
                <div key={role} style={{ border: `1.5px solid ${meta.bg}`, borderRadius: 12, padding: '14px 16px', background: meta.bg + '55' }}>
                  ...
                  <div style={{ display: 'grid', gap: 3 }}>
                    {meta.permissions.map((p, i) => (
                      <p key={i} style={{ fontSize: 12, color: '#4B5563' }}>
                        <span style={{ color: meta.color, fontWeight: 700 }}>✓</span> {p}
                      </p>
                    ))}
                  </div>
                </div>
